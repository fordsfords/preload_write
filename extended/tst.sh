...
B="./preload_write_test"   # Executable under test.
...
if [ "$SINGLE_T" -eq 0 -o "$SINGLE_T" -eq "$T" ]; then :
  TEST
  LD_LIBRARY_PATH="." LD_PRELOAD="./libpreload_write.so" $B -t $T 2>&1 | tee -a $B.$T.log;  ST=${PIPESTATUS[0]}; ASSRT "$ST -eq 0"
  OK
fi
...
if [ "$SINGLE_T" -eq 0 -o "$SINGLE_T" -eq "$T" ]; then :
  TEST
  LD_LIBRARY_PATH="." $B -t $T 2>&1 | tee -a $B.$T.log;  ST=${PIPESTATUS[0]}; ASSRT "$ST -eq 0"
  OK
fi
